...
export async function seatmapDetail(team, eventId, enableSeatIds, { delayMs = 700, log } = {}) {
  ...
  // Standing-room / unassigned tickets have no seat polygon (the site buckets
  // them under seat_map_id=unknown). They are often the cheapest, so collect
  // them too and surface them off-map rather than dropping them.
  let unassigned = null;
  const unk = await polygonListings(team, eventId, 'unknown', { delayMs });
  if (unk?.length) {
    const u = unk
      .filter((l) => !/高校生|中学生|小学生|こども|子ども|子供|シニア|女性限定|レディース|学割/.test(l.seat))
      .sort((a, b) => a.price - b.price);
    if (u.length) unassigned = { min: u[0].price, count: u.length, url: u[0].url || null };
  }

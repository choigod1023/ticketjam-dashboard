...
// misleading unless they are flagged.
...
/** Query string for the event listing pages: cheapest first, optional lot-size filter. */
function listingQuery(cfg) {
  // Sort cheapest-first so a truncated crawl still yields an exact minimum.
  const parts = ['sort_query%5BsortKindKey%5D=price_for_cell_asc'];
  const n = cfg.ticketCount;
  // The site's own "buyable in N tickets" filter — it also keeps larger lots
  // that the seller allows splitting, which a raw 枚 count would miss.
  if (Number.isInteger(n) && n >= 1 && n <= 5) {
    parts.push(`sort_query%5BremainingCountMultiple%5B${n}%5D%5D=1`);
  }
  return '?' + parts.join('&');
}
...
export async function refresh({ log = console.log } = {}) {
  ...
  for (const game of scoped) {
    try {
      ...
      const flagged = listings.map((l) => ({ ...l, restricted: RESTRICTED.test(l.seat) }));
      const stats = summarise(flagged.filter((l) => !l.restricted));
      const cheapest = [...flagged].sort((a, b) => a.price - b.price).slice(0, 40);
  ...
  const payload = {
    ...
    trip: cfg.trip,
    regions: cfg.regions,
    ticketCount: cfg.ticketCount ?? null,

...
import {
  ...
  readSeatmap, writeSeatmap, seatmapAge,
} from './store.js';
import { fetchResale } from './resale.js';
import { officialFor } from './official.js';
import { venueGeo, slugify, fetchEnableSeatIds } from './seatgeo.js';
import { seatmapDetail } from './seatmap_detail.js';
...
export async function refresh({ log = console.log } = {}) {
  ...
  // 2b. Refresh exact per-polygon seat-map pricing for ONE event per cycle
  //     (oldest first). Each event costs ~100 requests, so this is decoupled
  //     from the 10-min price loop and rotates through the seat-map venues.
  const detailTtlMs = (cfg.seatmapTtlHours ?? 1) * 3600_000;
  const geoGames = entries.filter((e) => e.meta.hasGeo);
  const stale = geoGames
    .map((e) => ({ e, age: seatmapAge(e.meta.id) }))
    .filter((x) => x.age >= detailTtlMs)
    .sort((a, b) => b.age - a.age);
  if (stale.length) {
    const { e } = stale[0];
    const g = { id: e.meta.id, teams: e.meta.teams };
    try {
      const esi = await fetchEnableSeatIds(g, { delayMs });
      if (esi.length) {
        log(`  seatmap detail: ${e.meta.name} (${esi.length} polygons)`);
        const detail = await seatmapDetail(g.teams[0], e.meta.id, esi, { delayMs, log });
        writeSeatmap(e.meta.id, detail);
        log(`  seatmap detail done: ${Object.keys(detail.polys).length} polygons + ${detail.unassigned?.count ?? 0} 입석`);
      }
    } catch (err) {
      errors.push(`seatmap ${e.meta.id}: ${err.message}`);
      log(`  ! seatmap ${e.meta.id}: ${err.message}`);
    }
  }
  ...
  const payload = {
    ...
    games: entries.map((e) => ({
      ...
      hasSeatDetail: !!readSeatmap(e.meta.id),

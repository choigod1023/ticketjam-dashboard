  ...
  if (stale.length) {
    ...
    try {
      ...
      if (esi.length) {
        ...
        const polys = await seatmapDetail(g.teams[0], e.meta.id, esi, { delayMs, log });
        writeSeatmap(e.meta.id, polys);
        log(`  seatmap detail done: ${Object.keys(polys).length} priced polygons`);
      }
    } catch (err) {
